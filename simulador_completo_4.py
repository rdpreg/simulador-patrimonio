...
if "valores" in st.session_state and "patrimonio_final" in st.session_state:
    if st.button("📄 Gerar PDF do Relatório"):
        # Salvar gráfico de linha em buffer
        buffer_linha = BytesIO()
        fig.write_image(buffer_linha, format="png")
        buffer_linha.seek(0)

        # Salvar gráfico de barras em buffer
